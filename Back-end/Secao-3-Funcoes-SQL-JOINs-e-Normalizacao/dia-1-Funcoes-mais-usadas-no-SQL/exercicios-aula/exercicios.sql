...
-- Utilizando a query anterior, faça as alterações para que seja exibido somente a quantidade de dinheiro necessária para cobrir a
...
SELECT department_id, COUNT(employee_id) AS `employees`, AVG(salary) AS `media_salarial` FROM employees
...
-- Escreva uma query que exiba as seguintes informações de cada funcionário: id, primeiro nome e dia do mês no qual foi contratado
-- (exiba somente o dia).
SELECT employee_id, first_name, DAY(hire_date) FROM employees;

-- Escreva uma query que exiba as seguintes informações de cada funcionário: id, primeiro nome e mês no qual foi contratado
-- (exiba somente o mês).
