...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {    
    ...
    // ```
    expect(objetoRetornado.fetchMenu()).toStrictEqual({ food: {
      'coxinha': 3.90, 
      'sanduiche': 9.90
    }, 
    drink: {
    ...
    // ```
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'agua', 'cerveja', 'sanduiche']);
    ...
    // ```
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'agua', 'cerveja', 'sanduiche', 'coxinha']);
    ...
    // ```
    expect(objetoRetornado.pay()).toBe(31.35);
